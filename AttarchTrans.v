...
Inductive useram_trans : stransition useram_state :=
  | useram_get_key : forall (acc: access component) (k: nat),
      acc useram read ->
      useram_trans
        (useram_key #acc ↦ k, useram_waiting_key)
        (useram_key #acc ↦ k, useram_waiting_request)
...
Inductive platam_trans : stransition platam_state :=
  | platam_measure_good_vm : platam_trans
      (⟨⟩, platam_init)
      (vm_ev #private platam ↦ good_vm_ev, platam_appraise)
  | platam_measure_bad_vm : platam_trans
      (⟨⟩, platam_init)
      (vm_ev #private platam ↦ bad_vm_ev, platam_appraise)
  | platam_release_key : forall (acc: access component),
      acc platam read ->
      platam_trans
        (vm_ev #acc ↦ good_vm_ev,
          platam_appraise)
        (vm_ev #acc ↦ good_vm_ev **
        useram_key #useram_key_acc ↦ useram_key_val,
          platam_run).
...
Inductive attarch_trans : stransition attarch_state :=
  | good_boot : attarch_trans 
      (⟨⟩, boot_from_good_plat)
      (boot_ev #readonly ↦ good_boot_ev, sel4_run (platam_init, vm_init))
  | bad_boot : attarch_trans 
      (⟨⟩, boot_from_bad_plat)
      (boot_ev #readonly ↦ bad_boot_ev, sel4_run (platam_init, vm_init))
